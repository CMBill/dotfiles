...
本仓库中的配置文件基于 Arch Linux，部分配置兼容 WSL 环境。
...
| `dotfiles/rime/`         | `~/.local/share/fcitx5/rime` | [Rime](https://rime.im/)                   | 中州韻输入法引擎配置（fcitx5 前端），子模块引入            |
...
> 脚本会自动创建 `~/.config/`、`~/.local/share/fcitx5/` 等必要目录。
...
- **Rime (fcitx5)**：需安装 `fcitx5-rime`，依赖 `fcitx5` 输入法框架；配置通过 git 子模块引入，克隆时需加 `--recursive`
